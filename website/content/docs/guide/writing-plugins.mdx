...
description: Guide for writing plugins for Pothos
---
...
1. `index.ts` which contains a plugin's actual implementation
...
1. A module augmentation of `@pothos/core/types`
...
2. An addition to the `Plugins` interface that maps the plugin name, to the plugin type \(this needs
   to be inside the `declare module` block\)
...
`global-types.ts` should NOT include definitions that do not belong to `@pothos/core/types`. Types
for your plugin should be added to a separate `types.ts` file, and imported as needed into
`global-types.ts`.

Because the augmented interfaces are module members rather than members of an ambient namespace,
sibling types are no longer in scope by bare name inside the `declare module` block. Import the
`PothosSchemaTypes` namespace from `@pothos/core` and qualify them \(`PothosSchemaTypes.ObjectRef`,
`PothosSchemaTypes.ObjectFieldOptions`, and so on\). Some of these names also exist as top-level
exports of `@pothos/core` with a different signature — `ObjectTypeOptions`, `InterfaceTypeOptions`
and `EnumTypeOptions` — so importing them by bare name resolves to the wrong type instead of
failing.
...
Each GraphQL type has its own options interface which can be extended. For example, to extend the
